...
ui <- shinyUI(dashboardPage(#skin = "blue" , # find appropriate uncg color?
...
server <- function(input,output){
  ...
  data_1.1 <- reactive({ # this is referenced in the ggplot. 
    data <- combined_results %>% filter(better_label == input$meter_choice_box_1) %>% 
      mutate("Time_Choice" = case_when(input$time_choice_box_1 =="year"~ as.character(year(Datetime)), 
                                       ...
                                       input$time_choice_box_1 =="hour"~ as.character(hm(Datetime)),
                                       T ~ as.character("Time_Choice"))) %>% 
      select("Actual","Predicted","Datetime","better_label", "Time_Choice") %>% 
      group_by(better_label,Time_Choice) %>%   # grouping by year, so this will be a year plot, could ask them for input
  ...
  output$task_1.1_plot <- renderPlot({
    ggplot(data_1.1(),aes(x = Time_Choice, color = better_label, group = 1)) + # ggplot, year on x axis
  ...
  output$meter_choice_plot_3 <- renderPlot({ # render a plot, the meter_choice_plot, which is found in task_1 tab
    ggplot(data_1(),aes(x = Time_Choice, color = better_label)) + # ggplot, year on x axis
